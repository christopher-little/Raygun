...
bool MeshShape::intersectTri(const Ray &r, float &t, Vector &p, Vector &n, float &u, float &v)
{
    ...
    for(unsigned int faceI=0; faceI<mesh->faces.size(); faceI+=3)
    {
        ...
        else
        {
            ...
            if(temp_t > epsilon && (nearest_t <= epsilon || temp_t < nearest_t))
            {
                nearest_t = temp_t;
...
bool MeshShape::intersectQuad(const Ray &r, float &t, Vector &p, Vector &n, float &u, float &v)
{
    ...
    if(nearest_quad >= 0)
    {
        ...
        if(std::abs(alpha_11 - 1.0f) < epsilon)
        {
            u = nearest_alpha;
            if(std::abs(beta_11 - 1.0f) < epsilon)
                v = nearest_beta;
            else
                v = nearest_beta / ((u*(beta_11 - 1.0f)) + 1.0f);
        }
        else if (std::abs(beta_11 - 1.0f) < epsilon)
        {
            v = nearest_beta;
